  ...
  describe 'show' do
    ...
    context 'logged out' do
      ...
      it 'returns unauthorized for a private discussion' do
        get :show, params: { id: private_discussion.id }
        expect(response.status).to eq 403
      end
    end
  end
  ...
  describe 'move' do
    ...
    context 'success' do
      it 'moves a discussion' do
        ...
        patch :move, id: discussion.id, group_id: destination_group.id
    ...
    context 'logged in' do
      ...
      context 'success' do
        ...
        it 'responds to a since parameter' do
          ...
          get :index, params: { group_id: group.id, since: 3.months.ago }
          json = JSON.parse(response.body)
          discussions = json['discussions'].map { |v| v['id'] }
        ...
        it 'responds to an until parameter' do
          four_months_ago = create :discussion, group: group, created_at: 4.months.ago, last_activity_at: 4.months.ago
          two_months_ago = create :discussion, group: group, created_at: 2.months.ago, last_activity_at: 2.months.ago
          get :index, params: { group_id: group.id, until: 3.months.ago }
          json = JSON.parse(response.body)
          discussions = json['discussions'].map { |v| v['id'] }
  ...
  describe 'set_volume' do
    ...
    context 'success' do
      it 'sets the volume of a thread' do
        ...
        put :set_volume, params: { id: discussion.id, volume: :mute }
    ...
    context 'failure' do
      it 'does not update a reader' do
        ...
        reader.update volume: :loud
        put :set_volume, params: { id: another_discussion.id, volume: :mute }
    ...
    context 'success' do
      ...
      describe 'make_announcement' do
        it 'does not email users for non-announcements' do
          expect { post :create, params: { discussion: discussion_params } }.to_not change { ActionMailer::Base.deliveries.count }
        end

        it 'makes an announcement' do
          discussion_params[:make_announcement] = true
          expect { post :create, params: { discussion: discussion_params } }.to change { ActionMailer::Base.deliveries.count }.by(1)
        end
      end
      ...
      describe 'mentioning' do
        it 'mentions appropriate users' do
          ...
          expect { post :create, params: { discussion: discussion_params } }.to change { Event.where(kind: :user_mentioned).count }.by(1)
        end

        it 'does not mention users not in the group' do
          discussion_params[:description] = "Hello, @#{another_user.username}!"
          expect { post :create, params: { discussion: discussion_params } }.to_not change { Event.where(kind: :user_mentioned).count }
        end
      end
    end
